...
def normalise_wundumgebung(val):
    """
    Normalisiert Wundumgebung basierend auf WUNDUMGEBUNG_GT_MAPPING.
    """
...
def normalise_wundrand(val):
    """
    Normalisiert Wundrand basierend auf WUNDRAND_GT_MAPPING.
    """
    return normalise_by_mapping(val, WUNDRAND_GT_MAPPING)


def normalise_sekundaerverband(val):
    """
    Normalisiert Sekundärverband basierend auf SEKUNDAERVERBAND_GT_MAPPING.
    """
    return normalise_by_mapping(val, SEKUNDAERVERBAND_GT_MAPPING)


def normalise_produkt(val):
    """
    Normalisiert Präferenz- und Alternativprodukte basierend auf PRODUKT_GT_MAPPING.
    """
...
# =====================================================================
# STATIC GT FILE NORMALIZATION
# =====================================================================
def normalize_gt_file(input_path: str, output_path: str):
    """
    Führt die Bereinigung auf die CSV-Datei aus und speichert sie am Zielort.
    """
    ...
    # Merge wundtyp and wundtyp_spezifikation
    if 'wundtyp' in df_cleaned.columns and 'wundtyp_spezifikation' in df_cleaned.columns:
        df_cleaned['wundtyp'] = df_cleaned.apply(
            lambda r: merge_wundtyp_sonstiges(r['wundtyp'], r['wundtyp_spezifikation']),
            axis=1
        )
    ...
    if 'sekundaerverband' in df_cleaned.columns:
        df_cleaned['sekundaerverband'] = df_cleaned['sekundaerverband'].apply(normalise_sekundaerverband)
...
def normalize_llm_file(input_path: str, output_path: str):
    ...
    if 'sekundaerverband_fixierung' in df_cleaned.columns:
        df_cleaned['sekundaerverband_fixierung'] = df_cleaned['sekundaerverband_fixierung'].apply(normalise_sekundaerverband)
        
    if 'praeferenz_verbandklasse' in df_cleaned.columns:
        df_cleaned['praeferenz_verbandklasse'] = df_cleaned['praeferenz_verbandklasse'].apply(normalise_produkt)
        
    if 'alternativ_verbandklasse' in df_cleaned.columns:
        df_cleaned['alternativ_verbandklasse'] = df_cleaned['alternativ_verbandklasse'].apply(normalise_produkt)
